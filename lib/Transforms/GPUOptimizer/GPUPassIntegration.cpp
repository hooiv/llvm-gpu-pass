...
void GPUPassIntegration::integrateWithScalarPasses(FunctionPassManager &FPM) {
  ...
  FPM.addPass(SROA());
  FPM.addPass(EarlyCSEPass(true));  // Include memorySSA
...
void GPUPassIntegration::integrateWithInliningPasses(ModulePassManager &MPM) {
  ...
  ModuleInlinerWrapperPass::InliningPolicy InlinePolicy;
  // Customize the policy for GPU code (e.g., more aggressive inlining)
  InlinePolicy.DefaultThreshold = 225;
  InlinePolicy.HotCallSiteThreshold = 400;
  
  // Add the inliner pass
  MPM.addPass(ModuleInlinerWrapperPass(InlinePolicy));
